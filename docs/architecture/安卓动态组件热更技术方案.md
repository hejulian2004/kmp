...
页面 ViewModel 发起布局数据获取，强制按照以下顺序进行降级容灾：
1. **1 级（内存缓存 RAM）**：ViewModel 作用域内已解析过的内存节点树直接复用。
2. **2 级（磁盘缓存 Disk）**：读取上一次静默请求并持久化在本地磁盘的最新 `JSON DSL`。
3. **3 级（包内默认打底 Asset）**：首刷离线或网络断开时，解压读取随 APK 打包发布的 `${module}_layout_default.json` 模板。
> **保障**：即便后端故障或 JSON 毁损，系统也会 100% 降级回第 3 级打底模板，保证 100% 渲染原生打包 UI，**绝对不崩溃、防空白**。
